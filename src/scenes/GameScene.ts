...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // 网格背景
    this.add.rectangle(GRID_X + BALANCE.gridCols * BALANCE.cellSize / 2, GRID_Y + BALANCE.gridRows * BALANCE.cellSize / 2, BALANCE.gridCols * BALANCE.cellSize, BALANCE.gridRows * BALANCE.cellSize, 0x000000, 0.25)
    ...
    this.add.text(W / 2, DROP_ZONE_H / 2, '点击此处投下弹珠 ↓', {
      fontFamily: 'JetBrains Mono, monospace', fontSize: '14px', color: '#f0b429',
    ...
    for (let i = 0; i < BALANCE.bottomSlots; i++) {
      ...
      this.add.text(x, SETTLE_Y + 20, isCenter ? '×2' : '×1', {
        fontFamily: 'JetBrains Mono, monospace', fontSize: '14px', color: isCenter ? '#f0b429' : '#2db7a3',
      }).setOrigin(0.5);
    }
    ...
    // 物理分组：弹珠 vs 钉子，用 collider 实现真实反弹
    ...
    // 边墙
    const leftWall = this.add.rectangle(0, H / 2, 4, H, 0x30363d).setOrigin(0, 0.5);
    this.physics.add.existing(leftWall, true);
    const rightWall = this.add.rectangle(W, H / 2, 4, H, 0x30363d).setOrigin(1, 0.5);
    ...
    this.comboDisplay = this.add.text(W - 20, 80, '', {
      fontFamily: 'JetBrains Mono, monospace', fontSize: '16px', color: '#f0b429',
  ...
  private drawBackground() {
    ...
    if (ch.id === 1) {
      ...
      for (let i = 0; i < 8; i++) {
        const x = i * (W / 8) + 20;
        bg.fillRect(x, H - 80, 60, 60);
        bg.fillStyle(0xf0b429, 0.5);
        bg.fillRect(x + 8, H - 60, 4, 4);
        bg.fillRect(x + 30, H - 60, 4, 4);
        bg.fillStyle(0x1a0a05, 1);
      }
      for (let i = 0; i < 60; i++) {
        bg.fillStyle(0xffffff, Math.random() * 0.6 + 0.2);
        bg.fillRect(Math.random() * W, Math.random() * (H - 200), 1, 1);
      }
    } else if (ch.id === 2) {
      for (let i = 0; i < 5; i++) {
        const x = (i + 1) * (W / 6);
        bg.fillStyle(0x2db7a3, 0.12);
      ...
      for (let i = 0; i < 30; i++) {
        bg.fillStyle(0xf0b429, Math.random() * 0.3);
        bg.fillRect(Math.random() * W, Math.random() * H, 2, 2);
      }
    } else if (ch.id >= 4) {
      for (let i = 0; i < 200; i++) {
        const c = Math.random() > 0.5 ? 0x2db7a3 : 0xffffff;
        bg.fillStyle(c, Math.random() * 0.7);
  ...
  private spawnBall(x: number, y: number, value: number, source: 'manual' | 'auto') {
    ...
    sprite.setCircle(7, 1, 1).setCollideWorldBounds(true).setBounce(0.55, 0.55).setVelocityY(50);
    ...
    const text = this.add.text(x, y - 14, formatNum(value), {
      fontFamily: 'JetBrains Mono, Courier New, monospace', fontSize: '11px', color: golden ? '#ffd700' : '#ffffff',
  ...
  private renderPeg(peg: PegSave) {
    ...
    sprite.setCircle(9, 2, 2).setImmovable(true);
    sprite.setBounce(0.6, 0.6);
    sprite.body!.moves = false;
    sprite.setDisplaySize(18, 18);

    const label = cfg.operator === '+' ? `+${Math.floor(cfg.operand + (peg.level - 1) * cfg.growth)}`
      : cfg.operator === '*' ? `×${(cfg.operand + (peg.level - 1) * cfg.growth).toFixed(1)}`
      : cfg.operator === '/' ? '÷2'
      : cfg.operator === '^' ? '^2'
      : cfg.operator === 'addPercent' ? `%`
      : cfg.operator === 'maxMul' ? '×2+'
      : '贤';
    const text = this.add.text(x, y, label, {
      fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: '#ffffff',
  ...
  private spawnFloatText(x: number, y: number, text: string, color: number) {
    const t = this.add.text(x, y, text, {
      fontFamily: 'JetBrains Mono, monospace', fontSize: '12px', color: '#' + color.toString(16).padStart(6, '0'),
      stroke: '#000', strokeThickness: 2,
    }).setOrigin(0.5);
